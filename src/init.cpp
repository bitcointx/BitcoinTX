...
void Shutdown()
{
    ...
    RenameThread("bitcointx-shutoff");
...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: bitcointx.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: bitcointxd.pid)") + "\n";
    ...
    strUsage += "  -port=<port>           " + _("Listen for connections on <port> (default: 28280 or testnet: 28281)") + "\n";
    ...
    strUsage += "  -rpcport=<port>        " + _("Listen for JSON-RPC connections on <port> (default: 28282 or testnet: 28283)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizebitcointxamount=<n> " + _("Keep N BitcoinTX anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. BitcoinTX is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. BitcoinTX is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("BitcoinTX version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon)
        fprintf(stdout, "BitcoinTX server starting\n");
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of BitcoinTX") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart BitcoinTX to complete") << "\n";
    ...
    nAnonymizeBitcoinTXAmount = GetArg("-anonymizebitcointxamount", 0);
    if(nAnonymizeBitcoinTXAmount > 999999) nAnonymizeBitcoinTXAmount = 999999;
    if(nAnonymizeBitcoinTXAmount < 2) nAnonymizeBitcoinTXAmount = 2;
    ...
    if(fEnableInstantX){
        ...
        if(nInstantXDepth < 0) nAnonymizeBitcoinTXAmount = 0;
    } else {
        nInstantXDepth = 0;
    }
    ...
    LogPrintf("Anonymize BitcoinTX Amount %d\n", nAnonymizeBitcoinTXAmount);

    /* Denominations
       A note about convertability. Within Darksend pools, each denomination
       is convertable to another.
       For example:
       1BTX+1000 == (.1BTX+100)*10
       10BTX+10000 == (1BTX+1000)*10
    */
