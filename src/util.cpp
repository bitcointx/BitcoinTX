...
int nAnonymizeBitcoinTXAmount = 500;
...
#else
    const char* pszModule = "bitcointx";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\BTX
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\BTX
    // Mac: ~/Library/Application Support/BTX
    // Unix: ~/.BTX
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "BTX";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "bitcointx.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "bitcointxd.pid"));
