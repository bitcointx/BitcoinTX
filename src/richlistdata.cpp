// Copyright (c) 2015 The BitcoinTX developers
		      ...
		      if(!fInvalid)
		      {
			    ...
			    { 
			        ...
			        if(ExtractDestination(atxout.scriptPubKey, dest))
			        {
			    	        ...
			    	        std::string bitcointxAddress = CBitcoinAddress(dest).ToString();
				        CRichListDataItem richListDataItem;
				        richListDataItem.sAddress = bitcointxAddress;
				        if(richListData.mapRichList.find(bitcointxAddress) == richListData.mapRichList.end())
				            richListData.mapRichList.insert(make_pair(bitcointxAddress, richListDataItem));
					
				        richListData.mapRichList[bitcointxAddress].nBalance = richListData.mapRichList[bitcointxAddress].nBalance - atxout.nValue;
