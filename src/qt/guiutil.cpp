...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("bitcointx"))
        return false;
...
{
    // Convert bitcointx:// to bitcointx:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("bitcointx://"))
    {
        uri.replace(0, 12, "bitcointx:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "BitcoinTX.lnk";
}
...
{
    return GetAutostartDir() / "bitcointx.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("BitcoinTX-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  bitcointx-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("BitcoinTX-Qt"));
