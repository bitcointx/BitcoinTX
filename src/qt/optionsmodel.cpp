...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeBitcoinTXAmount"))
        settings.setValue("nAnonymizeBitcoinTXAmount", 1000);

    nDarksendRounds = settings.value("nDarksendRounds").toLongLong();
    nAnonymizeBitcoinTXAmount = settings.value("nAnonymizeBitcoinTXAmount").toLongLong();
    ...
    if (settings.contains("nAnonymizeBitcoinTXAmount"))
        SoftSetArg("-anonymizebitcointxamount", settings.value("nAnonymizeBitcoinTXAmount").toString().toStdString());
}
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case anonymizeBitcoinTXAmount:
            nAnonymizeBitcoinTXAmount = value.toInt();
            settings.setValue("nAnonymizeBitcoinTXAmount", nAnonymizeBitcoinTXAmount);
            emit anonymizeBitcoinTXAmountChanged(nAnonymizeBitcoinTXAmount);
            break;
        default:
            break;
        }
