...
CBigNum bnProofOfStakeLimitV2(~uint256(0) >> 4);
...
const string strMessageMagic = "BitcoinTX Signed Message:\n";
...
int64_t GetProofOfStakeReward(int64_t nCoinAge, int64_t nFees, int64_t nValueIn, uint256 prevHash, int64_t nTime)
{
	...
	if (inputcoins >= 50000 && inputcoins < 100000 && rand <= 5) {
		nSubsidy += 5000 * COIN;
		LogPrint("creation", "BONUS stake: nSubsidy += 5000 * COIN, inputcoins =%s nCoinAge=%s rand=%s prevHash=%s\n", inputcoins, nCoinAge, rand, prevHash.ToString());
		}	
	else if (inputcoins >= 100000 && inputcoins < 300000 && rand <= 5){
		nSubsidy += 15000 * COIN;
		LogPrint("creation", "BONUS stake: nSubsidy += 15000 * COIN, inputcoins =%s nCoinAge=%s rand=%s prevHash=%s\n", inputcoins, nCoinAge, rand, prevHash.ToString());
		}	
...
{
    RenameThread("bitcointx-loadblk");
