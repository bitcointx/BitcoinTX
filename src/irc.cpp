...
void ThreadIRCSeed()
{
    ...
    RenameThread("bitcointx-ircseed");
...
void ThreadIRCSeed2()
{
	...
	int ntimeout = 1000;
        ...
        if (TestNet())
        {
            Send(hSocket, "JOIN #bitcointxcoinTEST\r");
            Send(hSocket, "WHO #bitcointxcoinTEST\r");
        } else
        {
	    LogPrintf("JOIN #bitcointxcoin\n");
            // randomly join #bitcointxcoin00-#bitcointxcoin05
            //int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #bitcointxcoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #bitcointxcoin%02d\r", channel_number).c_str());
        };
